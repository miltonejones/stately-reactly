...
const ComponentTreeView = ({ 
 ...
 return (
   <Layout data-testid="test-for-ComponentTreeView">
     ...
     {nodes?.map((node) => {
      const childComponents = components?.filter(f => f.componentID === node.ID).sort(sortByOrder);
      const ico = !!openComponents[node.ID] ? "Remove" : "Add";
        return (<Box key={node.ID}>
          <Flex sx={{ml: indent, pb: 0.5 }}>
            <TinyButton onClick={() => handleChange(node.ID)} icon={!!childComponents.length ? ico : "RadioButtonUnchecked"} /> 
            <TinyButton icon={library[node.ComponentType].Icon} /> 
            <Nowrap onClick={() => {
              handleChange(node.ID);
              handleSelect(node.ID)
            }} bold={selectedComponentID === node.ID} hover variant="body2">  
              {node.ComponentName} 
            </Nowrap> 
          </Flex> 
